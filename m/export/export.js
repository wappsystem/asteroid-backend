...
$('#participant_status__ID').on('change', function(){
	...
	switch(part_sel){
		case '0': part_query="";part_query2="Yes";
		break;
		case '1': part_query="Yes";part_query2="Yes";
		break;
	}
})
//--------------------------------------------------------
	...
	for(var j=1;j<bi_txt.length;j++){
		...
		for(var ii=0;ii<participant_rec.length;ii++){
			...
			for (var kk=0;kk<task_rec.length;kk++){
				if(task_rec[kk].Entry_ID==participant_rec[ii].ID){
...
var combine_output=function(output,participant){
	...
	for(var j=0;j<participant[0].length;j++){
		var enrolled=false;
		if(participant[0][j].Consent==part_query || participant[0][j].Consent==part_query2 ) enrolled=true;
		if(part_query=="") enrolled=true;
		console.log(participant[0][j].Consent+'    '+part_query+'    '+part_query2+'    '+enrolled)
		if((enrolled) ){
